...
CREATE TABLE IF NOT EXISTS `mydb`.`actions` (
  ...
  UNIQUE INDEX `Service_name_Unique` (`idServices` ASC, `name` ASC),
...
CREATE TABLE IF NOT EXISTS `mydb`.`signals` (
  ...
  UNIQUE INDEX `Services_Signale_Unique` (`idServices` ASC, `name` ASC),
  CONSTRAINT `FK_Services`
    FOREIGN KEY (`idServices`)
    REFERENCES `mydb`.`services` (`idServices`)
    ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS `mydb`.`users_connections` (
  ...
  INDEX `FK_Connections_idx` (`idConnections` ASC)  COMMENT 'C\'est de la merde',
  CONSTRAINT `FK_Users`
    FOREIGN KEY (`idUsers`)
